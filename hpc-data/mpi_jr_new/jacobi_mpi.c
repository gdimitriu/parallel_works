...
//JACOBI
	if(rank==0) for(i=0;i<dim;i++) xn_1[i]=ty[i]/mat[i][i];
	//compute q
	q1=0.0;
	for(i=1;i<dim;i++) q1+=fabs(matrix[0][i]/matrix[0][0]);
	for(i=1;i<partition;i++)
	{
		sum=0.0;
		for(j=0;j<dim;j++) if((firstrow+i)!=j) sum+=fabs(matrix[i][j]/matrix[i][i]);
		if(q1<sum) q1=sum;
	}
	MPI_Reduce(&q1,&q,1,MPI_DOUBLE,MPI_MAX,0,MPI_COMM_WORLD);
	MPI_Bcast(&q,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
	sum=fabs(matrix[0][dim]/matrix[0][0]);
	for(i=1;i<partition;i++)
	{
		sum=fabs(matrix[i][dim]/matrix[i][i]);
		if(sum1<sum) sum1=sum;
	}
	MPI_Reduce(&sum1,&sum,1,MPI_DOUBLE,MPI_MAX,0,MPI_COMM_WORLD);

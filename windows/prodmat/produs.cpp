...
#endif
//external function for thread

UINT calcul_th(LPVOID pParam)
{
	date *idata;
	idata=(date *)pParam;
	long i,j,k;
	for(i=idata->indexare;i<idata->dimensiune;i=i+idata->proc)
		for(j=0;j<idata->dimensiune;j++)
		{
			idata->matc[i][j]=0.0;
			for(k=0;k<idata->dimensiune;k++) idata->matc[i][j]+=idata->mata[i][k]*idata->matb[k][j];
		}
	return 0;
}
...
{
	int i;
	CWinThread *thread[64];
	date *dateth;
	dateth=new date[threads];
	hthreads=new HANDLE[threads];
	for(i=0;i<threads;i++)
	{
		dateth[i].indexare=i;
		dateth[i].dimensiune=dim;
		dateth[i].mata=mata;
		dateth[i].matb=matb;
		dateth[i].matc=matc;
		dateth[i].proc=threads;
		thread[i]=AfxBeginThread(calcul_th,(LPVOID)&(dateth[i]),THREAD_PRIORITY_NORMAL,0,CREATE_SUSPENDED);
		thread[i]->m_bAutoDelete=FALSE;
		hthreads[i]=thread[i]->m_hThread;
		thread[i]->ResumeThread();
	}
	//join
	::WaitForMultipleObjects(threads,hthreads,TRUE,INFINITE);
	for(i=0;i<threads;i++) delete thread[i];
	delete [] hthreads;
	delete [] dateth;
}

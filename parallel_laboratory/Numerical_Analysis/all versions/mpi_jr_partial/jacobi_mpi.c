...
double q,sum;
	...
	if(rank==0) xn=(double *)calloc(dim,sizeof(double));
	else xn=(double *)calloc(partition,sizeof(double));
...
//	MPI_Barrier(MPI_COMM_WORLD);
	...
	{	
		...
		if(rank==0)
		{
			...
			for(i=1;i<dim;i++) if(sum<fabs(xn[i]-xn_1[i])) sum=fabs(xn[i]-xn_1[i]);
			sum=sum*q/(1-q);
	...
	memcpy(tx,xn,dim*sizeof(double));
	free(xn);
	free(xn_1);
	free(*matrix);
	free(matrix);

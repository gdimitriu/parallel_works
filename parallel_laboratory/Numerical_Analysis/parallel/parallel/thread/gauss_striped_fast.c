...
void *thread_gauss_striped_fast_block(void *p)
{
	ssystem_striped_fast data=*(ssystem_striped_fast *)p;
	int s;
	s=data.N/data.P;
...
int gauss_striped_fast(int dim,int thread,double **mat,double *x,double *y,int type)
{
	/*
		...
		x is unknown vector
		y is free vector
		type is 0 for rowwise ciclic and 1 for rowwise block
		NOTE: the columnwise partioning doesn't exist
	*/
	pthread_t *pt;
	...
	*/
	for(i=0;i<thread;i++)
	{
		...
		switch(type)
		{
			...
			case 1:
			{
				if(pthread_create(&pt[i],&attr,thread_gauss_striped_fast_block,&data[i]))
				{
					perror("Can not create threads");
					free(pt);
					free(data);

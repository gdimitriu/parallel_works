...
MPI_Datatype datastruct_extreme;	/* la nouvele structure */
...
MPI_Datatype type_old[2];			
	...
	indices[1]-=indices[0];
	indices[0]=0;
	MPI_Type_struct(2,blocklens,indices,type_old,&datastruct_extreme);
	MPI_Type_commit(&datastruct_extreme);
	...
	*position=start;
	...
	{
		temp=data;
		MPI_Bcast(&temp,1,datastruct_extreme,i,MPI_COMM_WORLD);
	...
	*output=data.val;
	*position=data.pos;
	MPI_Type_free(&datastruct_extreme);
	return(0);
}
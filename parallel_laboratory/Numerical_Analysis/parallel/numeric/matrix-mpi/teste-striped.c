...
if(rank==0)	readfile(numea,cs);
   ciclique=(int *)calloc(partition,sizeof(int));
	...
	diffusion_sq_striped_ciclique_mpi(rank,size,partition,cs,a,ciclique,N,0);
if(rank==0)	readfile(numea,cs);	
	diffusion_sq_striped_ciclique_mpi(rank,size,partition,cs,b,ciclique,N,1);
	MPI_Barrier(MPI_COMM_WORLD);
	MPI_Finalize();
	exit(0);
}

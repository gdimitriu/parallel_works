...
void diffusion_donne_check_mpi(MPI_Comm mesh2_comm,int size,int rank,double **matin,double **matout,int partition);
/*
...
void diffusion_gauss_striped_ciclique_mpi(int rank,int size,double **mat,double **augumente,int *ciclique,int variable,int type);
/*
	rank c'est le rank de processor
	size c'est le numero des processerus
	mat c'est la matrice complete dans le processe principale

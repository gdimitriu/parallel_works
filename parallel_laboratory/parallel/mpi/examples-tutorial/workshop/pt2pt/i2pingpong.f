      ...
      double precision bufsend(maxn), bufrecv(maxn), t0, t1, tmin
      integer i, j, k
      ...
      do 10 i=1,5
         ...
         do 20 j=1,ntests
            ...
            if (rank .eq. 0) then
               ...
               t0 = MPI_WTIME()
               do 30 k=1,nloop
                  call MPI_ISEND( bufsend, isizes(i),
     $                 MPI_DOUBLE_PRECISION, dest, tag, MPI_COMM_WORLD,
     $                 request(1), ierr )
                  call MPI_IRECV( bufrecv, isizes(i),
     $                 MPI_DOUBLE_PRECISION, dest, tag, MPI_COMM_WORLD,
     $                 request(2), ierr )

...
static long MPI_Send_nbytes_0;
static long MPI_Bsend_nbytes_0;
static long MPI_Isend_nbytes_0;
...
int  MPI_Finalize(  )
{
  int  returnVal;
    ...
    if (MPI_Allgather_ncalls_0) {
      printf( "MPI_Allgather: %d calls\n",
               MPI_Allgather_ncalls_0 );
    }
  
    if (MPI_Allgatherv_ncalls_0) {
      printf( "MPI_Allgatherv: %d calls\n",
               MPI_Allgatherv_ncalls_0 );
    }
  
    if (MPI_Allreduce_ncalls_0) {
      printf( "MPI_Allreduce: %d calls\n",
               MPI_Allreduce_ncalls_0 );
    }
  
    if (MPI_Alltoall_ncalls_0) {
      printf( "MPI_Alltoall: %d calls\n",
               MPI_Alltoall_ncalls_0 );
    }
  
    if (MPI_Alltoallv_ncalls_0) {
      printf( "MPI_Alltoallv: %d calls\n",
               MPI_Alltoallv_ncalls_0 );
    }
  
    if (MPI_Barrier_ncalls_0) {
      printf( "MPI_Barrier: %d calls\n",
               MPI_Barrier_ncalls_0 );
    }
  
    if (MPI_Bcast_ncalls_0) {
      printf( "MPI_Bcast: %d calls\n",
               MPI_Bcast_ncalls_0 );
    }
  
    if (MPI_Gather_ncalls_0) {
      printf( "MPI_Gather: %d calls\n",
               MPI_Gather_ncalls_0 );
    }
  
    if (MPI_Gatherv_ncalls_0) {
      printf( "MPI_Gatherv: %d calls\n",
               MPI_Gatherv_ncalls_0 );
    }
  
    if (MPI_Op_create_ncalls_0) {
      printf( "MPI_Op_create: %d calls\n",
               MPI_Op_create_ncalls_0 );
    }
  
    if (MPI_Op_free_ncalls_0) {
      printf( "MPI_Op_free: %d calls\n",
               MPI_Op_free_ncalls_0 );
    }
  
    if (MPI_Reduce_scatter_ncalls_0) {
      printf( "MPI_Reduce_scatter: %d calls\n",
               MPI_Reduce_scatter_ncalls_0 );
    }
  
    if (MPI_Reduce_ncalls_0) {
      printf( "MPI_Reduce: %d calls\n",
               MPI_Reduce_ncalls_0 );
    }
  
    if (MPI_Scan_ncalls_0) {
      printf( "MPI_Scan: %d calls\n",
               MPI_Scan_ncalls_0 );
    }
  
    if (MPI_Scatter_ncalls_0) {
      printf( "MPI_Scatter: %d calls\n",
               MPI_Scatter_ncalls_0 );
    }
  
    if (MPI_Scatterv_ncalls_0) {
      printf( "MPI_Scatterv: %d calls\n",
               MPI_Scatterv_ncalls_0 );
    }
  
    if (MPI_Attr_delete_ncalls_0) {
      printf( "MPI_Attr_delete: %d calls\n",
               MPI_Attr_delete_ncalls_0 );
    }
  
    if (MPI_Attr_get_ncalls_0) {
      printf( "MPI_Attr_get: %d calls\n",
               MPI_Attr_get_ncalls_0 );
    }
  
    if (MPI_Attr_put_ncalls_0) {
      printf( "MPI_Attr_put: %d calls\n",
               MPI_Attr_put_ncalls_0 );
    }
  
    if (MPI_Comm_compare_ncalls_0) {
      printf( "MPI_Comm_compare: %d calls\n",
               MPI_Comm_compare_ncalls_0 );
    }
  
    if (MPI_Comm_create_ncalls_0) {
      printf( "MPI_Comm_create: %d calls\n",
               MPI_Comm_create_ncalls_0 );
    }
  
    if (MPI_Comm_dup_ncalls_0) {
      printf( "MPI_Comm_dup: %d calls\n",
               MPI_Comm_dup_ncalls_0 );
    }
  
    if (MPI_Comm_free_ncalls_0) {
      printf( "MPI_Comm_free: %d calls\n",
               MPI_Comm_free_ncalls_0 );
    }
  
    if (MPI_Comm_group_ncalls_0) {
      printf( "MPI_Comm_group: %d calls\n",
               MPI_Comm_group_ncalls_0 );
    }
  
    if (MPI_Comm_rank_ncalls_0) {
      printf( "MPI_Comm_rank: %d calls\n",
               MPI_Comm_rank_ncalls_0 );
    }
  
    if (MPI_Comm_remote_group_ncalls_0) {
      printf( "MPI_Comm_remote_group: %d calls\n",
               MPI_Comm_remote_group_ncalls_0 );
    }
  
    if (MPI_Comm_remote_size_ncalls_0) {
      printf( "MPI_Comm_remote_size: %d calls\n",
               MPI_Comm_remote_size_ncalls_0 );
    }
  
    if (MPI_Comm_size_ncalls_0) {
      printf( "MPI_Comm_size: %d calls\n",
               MPI_Comm_size_ncalls_0 );
    }
  
    if (MPI_Comm_split_ncalls_0) {
      printf( "MPI_Comm_split: %d calls\n",
               MPI_Comm_split_ncalls_0 );
    }
  
    if (MPI_Comm_test_inter_ncalls_0) {
      printf( "MPI_Comm_test_inter: %d calls\n",
               MPI_Comm_test_inter_ncalls_0 );
    }
  
    if (MPI_Group_compare_ncalls_0) {
      printf( "MPI_Group_compare: %d calls\n",
               MPI_Group_compare_ncalls_0 );
    }
  
    if (MPI_Group_difference_ncalls_0) {
      printf( "MPI_Group_difference: %d calls\n",
               MPI_Group_difference_ncalls_0 );
    }
  
    if (MPI_Group_excl_ncalls_0) {
      printf( "MPI_Group_excl: %d calls\n",
               MPI_Group_excl_ncalls_0 );
    }
  
    if (MPI_Group_free_ncalls_0) {
      printf( "MPI_Group_free: %d calls\n",
               MPI_Group_free_ncalls_0 );
    }
  
    if (MPI_Group_incl_ncalls_0) {
      printf( "MPI_Group_incl: %d calls\n",
               MPI_Group_incl_ncalls_0 );
    }
  
    if (MPI_Group_intersection_ncalls_0) {
      printf( "MPI_Group_intersection: %d calls\n",
               MPI_Group_intersection_ncalls_0 );
    }
  
    if (MPI_Group_rank_ncalls_0) {
      printf( "MPI_Group_rank: %d calls\n",
               MPI_Group_rank_ncalls_0 );
    }
  
    if (MPI_Group_range_excl_ncalls_0) {
      printf( "MPI_Group_range_excl: %d calls\n",
               MPI_Group_range_excl_ncalls_0 );
    }
  
    if (MPI_Group_range_incl_ncalls_0) {
      printf( "MPI_Group_range_incl: %d calls\n",
               MPI_Group_range_incl_ncalls_0 );
    }
  
    if (MPI_Group_size_ncalls_0) {
      printf( "MPI_Group_size: %d calls\n",
               MPI_Group_size_ncalls_0 );
    }
  
    if (MPI_Group_translate_ranks_ncalls_0) {
      printf( "MPI_Group_translate_ranks: %d calls\n",
               MPI_Group_translate_ranks_ncalls_0 );
    }
  
    if (MPI_Group_union_ncalls_0) {
      printf( "MPI_Group_union: %d calls\n",
               MPI_Group_union_ncalls_0 );
    }
  
    if (MPI_Intercomm_create_ncalls_0) {
      printf( "MPI_Intercomm_create: %d calls\n",
               MPI_Intercomm_create_ncalls_0 );
    }
  
    if (MPI_Intercomm_merge_ncalls_0) {
      printf( "MPI_Intercomm_merge: %d calls\n",
               MPI_Intercomm_merge_ncalls_0 );
    }
  
    if (MPI_Keyval_create_ncalls_0) {
      printf( "MPI_Keyval_create: %d calls\n",
               MPI_Keyval_create_ncalls_0 );
    }
  
    if (MPI_Keyval_free_ncalls_0) {
      printf( "MPI_Keyval_free: %d calls\n",
               MPI_Keyval_free_ncalls_0 );
    }
  
    if (MPI_Abort_ncalls_0) {
      printf( "MPI_Abort: %d calls\n",
               MPI_Abort_ncalls_0 );
    }
  
    if (MPI_Error_class_ncalls_0) {
      printf( "MPI_Error_class: %d calls\n",
               MPI_Error_class_ncalls_0 );
    }
  
    if (MPI_Errhandler_create_ncalls_0) {
      printf( "MPI_Errhandler_create: %d calls\n",
               MPI_Errhandler_create_ncalls_0 );
    }
  
    if (MPI_Errhandler_free_ncalls_0) {
      printf( "MPI_Errhandler_free: %d calls\n",
               MPI_Errhandler_free_ncalls_0 );
    }
  
    if (MPI_Errhandler_get_ncalls_0) {
      printf( "MPI_Errhandler_get: %d calls\n",
               MPI_Errhandler_get_ncalls_0 );
    }
  
    if (MPI_Error_string_ncalls_0) {
      printf( "MPI_Error_string: %d calls\n",
               MPI_Error_string_ncalls_0 );
    }
  
    if (MPI_Errhandler_set_ncalls_0) {
      printf( "MPI_Errhandler_set: %d calls\n",
               MPI_Errhandler_set_ncalls_0 );
    }
  
    if (MPI_Finalize_ncalls_0) {
      printf( "MPI_Finalize: %d calls\n",
               MPI_Finalize_ncalls_0 );
    }
  
    if (MPI_Get_processor_name_ncalls_0) {
      printf( "MPI_Get_processor_name: %d calls\n",
               MPI_Get_processor_name_ncalls_0 );
    }
  
    if (MPI_Init_ncalls_0) {
      printf( "MPI_Init: %d calls\n",
               MPI_Init_ncalls_0 );
    }
  
    if (MPI_Initialized_ncalls_0) {
      printf( "MPI_Initialized: %d calls\n",
               MPI_Initialized_ncalls_0 );
    }
  
    if (MPI_Wtick_ncalls_0) {
      printf( "MPI_Wtick: %d calls\n",
               MPI_Wtick_ncalls_0 );
    }
  
    if (MPI_Wtime_ncalls_0) {
      printf( "MPI_Wtime: %d calls\n",
               MPI_Wtime_ncalls_0 );
    }
  
    if (MPI_Address_ncalls_0) {
      printf( "MPI_Address: %d calls\n",
               MPI_Address_ncalls_0 );
    }
  
    if (MPI_Bsend_ncalls_0) {
      printf( "MPI_Bsend: %d calls\n",
               MPI_Bsend_ncalls_0 );
    }
  
    if (MPI_Bsend_init_ncalls_0) {
      printf( "MPI_Bsend_init: %d calls\n",
               MPI_Bsend_init_ncalls_0 );
    }
  
    if (MPI_Buffer_attach_ncalls_0) {
      printf( "MPI_Buffer_attach: %d calls\n",
               MPI_Buffer_attach_ncalls_0 );
    }
  
    if (MPI_Buffer_detach_ncalls_0) {
      printf( "MPI_Buffer_detach: %d calls\n",
               MPI_Buffer_detach_ncalls_0 );
    }
  
    if (MPI_Cancel_ncalls_0) {
      printf( "MPI_Cancel: %d calls\n",
               MPI_Cancel_ncalls_0 );
    }
  
    if (MPI_Request_free_ncalls_0) {
      printf( "MPI_Request_free: %d calls\n",
               MPI_Request_free_ncalls_0 );
    }
  
    if (MPI_Recv_init_ncalls_0) {
      printf( "MPI_Recv_init: %d calls\n",
               MPI_Recv_init_ncalls_0 );
    }
  
    if (MPI_Send_init_ncalls_0) {
      printf( "MPI_Send_init: %d calls\n",
               MPI_Send_init_ncalls_0 );
    }
  
    if (MPI_Get_elements_ncalls_0) {
      printf( "MPI_Get_elements: %d calls\n",
               MPI_Get_elements_ncalls_0 );
    }
  
    if (MPI_Get_count_ncalls_0) {
      printf( "MPI_Get_count: %d calls\n",
               MPI_Get_count_ncalls_0 );
    }
  
    if (MPI_Ibsend_ncalls_0) {
      printf( "MPI_Ibsend: %d calls\n",
               MPI_Ibsend_ncalls_0 );
    }
  
    if (MPI_Iprobe_ncalls_0) {
      printf( "MPI_Iprobe: %d calls\n",
               MPI_Iprobe_ncalls_0 );
    }
  
    if (MPI_Irecv_ncalls_0) {
      printf( "MPI_Irecv: %d calls\n",
               MPI_Irecv_ncalls_0 );
    }
  
    if (MPI_Irsend_ncalls_0) {
      printf( "MPI_Irsend: %d calls\n",
               MPI_Irsend_ncalls_0 );
    }
  
    if (MPI_Isend_ncalls_0) {
      printf( "MPI_Isend: %d calls\n",
               MPI_Isend_ncalls_0 );
    }
  
    if (MPI_Issend_ncalls_0) {
      printf( "MPI_Issend: %d calls\n",
               MPI_Issend_ncalls_0 );
    }
  
    if (MPI_Pack_ncalls_0) {
      printf( "MPI_Pack: %d calls\n",
               MPI_Pack_ncalls_0 );
    }
  
    if (MPI_Pack_size_ncalls_0) {
      printf( "MPI_Pack_size: %d calls\n",
               MPI_Pack_size_ncalls_0 );
    }
  
    if (MPI_Probe_ncalls_0) {
      printf( "MPI_Probe: %d calls\n",
               MPI_Probe_ncalls_0 );
    }
  
    if (MPI_Recv_ncalls_0) {
      printf( "MPI_Recv: %d calls\n",
               MPI_Recv_ncalls_0 );
    }
  
    if (MPI_Rsend_ncalls_0) {
      printf( "MPI_Rsend: %d calls\n",
               MPI_Rsend_ncalls_0 );
    }
  
    if (MPI_Rsend_init_ncalls_0) {
      printf( "MPI_Rsend_init: %d calls\n",
               MPI_Rsend_init_ncalls_0 );
    }
  
    if (MPI_Send_ncalls_0) {
      printf( "MPI_Send: %d calls\n",
               MPI_Send_ncalls_0 );
    }
  
    if (MPI_Sendrecv_ncalls_0) {
      printf( "MPI_Sendrecv: %d calls\n",
               MPI_Sendrecv_ncalls_0 );
    }
  
    if (MPI_Sendrecv_replace_ncalls_0) {
      printf( "MPI_Sendrecv_replace: %d calls\n",
               MPI_Sendrecv_replace_ncalls_0 );
    }
  
    if (MPI_Ssend_ncalls_0) {
      printf( "MPI_Ssend: %d calls\n",
               MPI_Ssend_ncalls_0 );
    }
  
    if (MPI_Ssend_init_ncalls_0) {
      printf( "MPI_Ssend_init: %d calls\n",
               MPI_Ssend_init_ncalls_0 );
    }
  
    if (MPI_Start_ncalls_0) {
      printf( "MPI_Start: %d calls\n",
               MPI_Start_ncalls_0 );
    }
  
    if (MPI_Startall_ncalls_0) {
      printf( "MPI_Startall: %d calls\n",
               MPI_Startall_ncalls_0 );
    }
  
    if (MPI_Test_ncalls_0) {
      printf( "MPI_Test: %d calls\n",
               MPI_Test_ncalls_0 );
    }
  
    if (MPI_Testall_ncalls_0) {
      printf( "MPI_Testall: %d calls\n",
               MPI_Testall_ncalls_0 );
    }
  
    if (MPI_Testany_ncalls_0) {
      printf( "MPI_Testany: %d calls\n",
               MPI_Testany_ncalls_0 );
    }
  
    if (MPI_Test_cancelled_ncalls_0) {
      printf( "MPI_Test_cancelled: %d calls\n",
               MPI_Test_cancelled_ncalls_0 );
    }
  
    if (MPI_Testsome_ncalls_0) {
      printf( "MPI_Testsome: %d calls\n",
               MPI_Testsome_ncalls_0 );
    }
  
    if (MPI_Type_commit_ncalls_0) {
      printf( "MPI_Type_commit: %d calls\n",
               MPI_Type_commit_ncalls_0 );
    }
  
    if (MPI_Type_contiguous_ncalls_0) {
      printf( "MPI_Type_contiguous: %d calls\n",
               MPI_Type_contiguous_ncalls_0 );
    }
  
    if (MPI_Type_count_ncalls_0) {
      printf( "MPI_Type_count: %d calls\n",
               MPI_Type_count_ncalls_0 );
    }
  
    if (MPI_Type_extent_ncalls_0) {
      printf( "MPI_Type_extent: %d calls\n",
               MPI_Type_extent_ncalls_0 );
    }
  
    if (MPI_Type_free_ncalls_0) {
      printf( "MPI_Type_free: %d calls\n",
               MPI_Type_free_ncalls_0 );
    }
  
    if (MPI_Type_hindexed_ncalls_0) {
      printf( "MPI_Type_hindexed: %d calls\n",
               MPI_Type_hindexed_ncalls_0 );
    }
  
    if (MPI_Type_hvector_ncalls_0) {
      printf( "MPI_Type_hvector: %d calls\n",
               MPI_Type_hvector_ncalls_0 );
    }
  
    if (MPI_Type_indexed_ncalls_0) {
      printf( "MPI_Type_indexed: %d calls\n",
               MPI_Type_indexed_ncalls_0 );
    }
  
    if (MPI_Type_lb_ncalls_0) {
      printf( "MPI_Type_lb: %d calls\n",
               MPI_Type_lb_ncalls_0 );
    }
  
    if (MPI_Type_size_ncalls_0) {
      printf( "MPI_Type_size: %d calls\n",
               MPI_Type_size_ncalls_0 );
    }
  
    if (MPI_Type_struct_ncalls_0) {
      printf( "MPI_Type_struct: %d calls\n",
               MPI_Type_struct_ncalls_0 );
    }
  
    if (MPI_Type_ub_ncalls_0) {
      printf( "MPI_Type_ub: %d calls\n",
               MPI_Type_ub_ncalls_0 );
    }
  
    if (MPI_Type_vector_ncalls_0) {
      printf( "MPI_Type_vector: %d calls\n",
               MPI_Type_vector_ncalls_0 );
    }
  
    if (MPI_Unpack_ncalls_0) {
      printf( "MPI_Unpack: %d calls\n",
               MPI_Unpack_ncalls_0 );
    }
  
    if (MPI_Wait_ncalls_0) {
      printf( "MPI_Wait: %d calls\n",
               MPI_Wait_ncalls_0 );
    }
  
    if (MPI_Waitall_ncalls_0) {
      printf( "MPI_Waitall: %d calls\n",
               MPI_Waitall_ncalls_0 );
    }
  
    if (MPI_Waitany_ncalls_0) {
      printf( "MPI_Waitany: %d calls\n",
               MPI_Waitany_ncalls_0 );
    }
  
    if (MPI_Waitsome_ncalls_0) {
      printf( "MPI_Waitsome: %d calls\n",
               MPI_Waitsome_ncalls_0 );
    }
  
    if (MPI_Cart_coords_ncalls_0) {
      printf( "MPI_Cart_coords: %d calls\n",
               MPI_Cart_coords_ncalls_0 );
    }
  
    if (MPI_Cart_create_ncalls_0) {
      printf( "MPI_Cart_create: %d calls\n",
               MPI_Cart_create_ncalls_0 );
    }
  
    if (MPI_Cart_get_ncalls_0) {
      printf( "MPI_Cart_get: %d calls\n",
               MPI_Cart_get_ncalls_0 );
    }
  
    if (MPI_Cart_map_ncalls_0) {
      printf( "MPI_Cart_map: %d calls\n",
               MPI_Cart_map_ncalls_0 );
    }
  
    if (MPI_Cart_rank_ncalls_0) {
      printf( "MPI_Cart_rank: %d calls\n",
               MPI_Cart_rank_ncalls_0 );
    }
  
    if (MPI_Cart_shift_ncalls_0) {
      printf( "MPI_Cart_shift: %d calls\n",
               MPI_Cart_shift_ncalls_0 );
    }
  
    if (MPI_Cart_sub_ncalls_0) {
      printf( "MPI_Cart_sub: %d calls\n",
               MPI_Cart_sub_ncalls_0 );
    }
  
    if (MPI_Cartdim_get_ncalls_0) {
      printf( "MPI_Cartdim_get: %d calls\n",
               MPI_Cartdim_get_ncalls_0 );
    }
  
    if (MPI_Dims_create_ncalls_0) {
      printf( "MPI_Dims_create: %d calls\n",
               MPI_Dims_create_ncalls_0 );
    }
  
    if (MPI_Graph_create_ncalls_0) {
      printf( "MPI_Graph_create: %d calls\n",
               MPI_Graph_create_ncalls_0 );
    }
  
    if (MPI_Graph_get_ncalls_0) {
      printf( "MPI_Graph_get: %d calls\n",
               MPI_Graph_get_ncalls_0 );
    }
  
    if (MPI_Graph_map_ncalls_0) {
      printf( "MPI_Graph_map: %d calls\n",
               MPI_Graph_map_ncalls_0 );
    }
  
    if (MPI_Graph_neighbors_ncalls_0) {
      printf( "MPI_Graph_neighbors: %d calls\n",
               MPI_Graph_neighbors_ncalls_0 );
    }
  
    if (MPI_Graph_neighbors_count_ncalls_0) {
      printf( "MPI_Graph_neighbors_count: %d calls\n",
               MPI_Graph_neighbors_count_ncalls_0 );
    }
  
    if (MPI_Graphdims_get_ncalls_0) {
      printf( "MPI_Graphdims_get: %d calls\n",
               MPI_Graphdims_get_ncalls_0 );
    }
  
    if (MPI_Topo_test_ncalls_0) {
      printf( "MPI_Topo_test: %d calls\n",
               MPI_Topo_test_ncalls_0 );
    }
      
  if (MPI_Send_ncalls_0) {
       printf( "%d bytes send in %d calls with MPI_Send\n", 
	       MPI_Send_nbytes_0, MPI_Send_ncalls_0 );
       }
...
MPI_Comm comm;
{
  ...
  MPI_Bsend_nbytes_0+=typesize*count
  MPI_Bsend_ncalls_0++;
...
MPI_Request * request;
{
  ...
  MPI_Type_size( datatype, (MPI_Aint *)&typesize );
  MPI_Isend_nbytes_0+=typesize*count
  MPI_Isend_ncalls_0++;
...
MPI_Comm comm;
{
  ...
  MPI_Type_size( datatype, (MPI_Aint *)&typesize );
  MPI_Send_nbytes_0+=typesize*count
  MPI_Send_ncalls_0++;

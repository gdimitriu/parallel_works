...
if(rank!=0)	px=(double *)calloc(partition,sizeof(double));
	derniere=diffusion_gauss_striped_ciclique_mpi_sth(rank,size,tmat,ty,augumente,ciclique,variable);
if(rank==0) gauss_striped_ciclique_mpi_sth(rank,size,partition,ciclique,derniere,augumente,tx,variable);
else 	gauss_striped_ciclique_mpi_sth(rank,size,partition,ciclique,derniere,augumente,px,variable);		
/*
if(rank!=0)
{	
	for(i=0;i<partition;i++)

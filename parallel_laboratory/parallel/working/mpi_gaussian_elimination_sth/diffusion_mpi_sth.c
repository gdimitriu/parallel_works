...
#include<stdlib.h>
#include<mpi.h>
st_join_counter_t *diffusion_sth;
...
int diffusion_gauss_sc_mpi_sth(int rank,int size,double **mat,double *ty,double **augumente,int *ciclique,int variable)
/*
...
if(rank==0)
{
   	...
   	st_join_counter_destroy(diffusion_sth);
   	free(diffusion_sth);
	 	m=partition;
   	if(nr>0)
   	{

...
int main(int argc,char **argv)
{
	...
	if(rank==0)
	{
		for(i=0;i<dim;i++)
		{
			...
			{
				mat[i][j]=(double)rand();
				if(((double)rand()/(double)RAND_MAX)<0.5) mat[i][j]=-mat[i][j];
				mat[j][i]=mat[i][j];
			}
			mat[i][i]=(double)rand();
			if(((double)rand()/(double)RAND_MAX)<0.5) mat[i][i]=-mat[i][i];
	...
	if(rank==0) 
	{
		...
		if(rank==0) for(i=0;i<dim;i++) {if(abs(rez[i]-x[i])>1E-5) printf("%lf=%lf\n",rez[i],x[i]); fflush(stdout);}

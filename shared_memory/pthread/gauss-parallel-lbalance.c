...
void *thread_gauss_striped_fast_ciclic(void *p)
{
	...
	*/
	...
	{	
		...
		for(k=replay;k<i;k++)
		{
			while(replay<k && data.counters[k]==0 && proccount1<data.N)
			{
				for(j=replay+1;j<data.N;j++)
					data.mat[proccount1][j]-=data.mat[proccount1][replay]*data.mat[replay][j];
				data.y[proccount1]-=data.mat[proccount1][replay]*data.y[replay];
				data.mat[proccount1][replay]=0.0;
				proccount1+=data.P;
				if(proccount1>=data.N)
				{
					proccount1=i+data.P;
					test=0;
					replay++;
